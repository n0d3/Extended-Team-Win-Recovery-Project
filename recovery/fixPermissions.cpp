...
int fixPermissions::fixPerms(bool enable_debug, bool remove_data_for_missing_apps) {
	...
	if (!(TWFunc::Path_Exists(packageFile))) {
		...
		ui_print("(/data/system/packages.xml not found)\n\n");
		if (TWFunc::Path_Exists("/data/app")) {
			ui_print("Fresh installation of Rom detected.\n");
			ui_print("No need to 'Fix Permissions'.\n\n");
		} else {
			ui_print("Possible reasons:\n");
			ui_print("(a)TWRP detected /data but it's empty.\n");
			ui_print("(b)TWRP can't mount /data properly.\n\n");
		}
		return -1;
	}
	...
	ui_print("Fixing /data/app permissions...\n");
	if ((fixDataApps()) != 0) {
		return -1;
	}

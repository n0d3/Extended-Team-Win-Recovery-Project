...
ifeq ($(TW_INCLUDE_EXFAT), true)
    ...
    RELINK_SOURCE_FILES += $(TARGET_RECOVERY_ROOT_OUT)/sbin/exfatfsck
    RELINK_SOURCE_FILES += $(TARGET_RECOVERY_ROOT_OUT)/sbin/exfatlabel
    RELINK_SOURCE_FILES += $(TARGET_RECOVERY_ROOT_OUT)/sbin/dumpexfat
...
ifeq ($(TW_INCLUDE_NILFS2), true)
	#nilfs_cleanerd.conf
	include $(CLEAR_VARS)
	LOCAL_MODULE := nilfs_cleanerd.conf
	LOCAL_MODULE_TAGS := eng
	LOCAL_MODULE_CLASS := RECOVERY_EXECUTABLES
	LOCAL_MODULE_PATH := $(TARGET_RECOVERY_ROOT_OUT)/etc
	LOCAL_SRC_FILES := $(LOCAL_MODULE)
...
# Add the prerecoveryboot.sh so that 
# if needed, the recovery.fstab can be changed before processed at boot
include $(CLEAR_VARS)
LOCAL_MODULE := prerecoveryboot.sh
LOCAL_MODULE_TAGS := eng
LOCAL_MODULE_CLASS := RECOVERY_EXECUTABLES
LOCAL_MODULE_PATH := $(TARGET_RECOVERY_ROOT_OUT)/sbin
LOCAL_SRC_FILES := $(LOCAL_MODULE)
...
include $(CLEAR_VARS)
LOCAL_MODULE := split_nandroid_md5.sh
LOCAL_MODULE_TAGS := eng
LOCAL_MODULE_CLASS := RECOVERY_EXECUTABLES
LOCAL_MODULE_PATH := $(TARGET_RECOVERY_ROOT_OUT)/sbin
LOCAL_SRC_FILES := $(LOCAL_MODULE)
